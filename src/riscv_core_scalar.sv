...
module riscv_core_scalar (
   ...
   /* Import the ISA field types, and the argument to ecall to halt the
    ...
    register #($bits(ctrl_signals_E), 'b0) CTRL_E_M1_REG(.clk, .rst_l, .en(~halted & ~mem_stall), .clear(1'b0), .D(ctrl_signals_E), .Q(ctrl_signals_M1));
    ...
    assign data_offset = alu_out_M1[3:2];
    assign data_addr_aligned = {alu_out_M1[31:4], 2'b0};
    assign data_addr = MemRead_M1 ? data_addr_aligned : alu_out_M1[31:2];
    ...
    assign data_byte_M1 = alu_out_M1[1:0];
    ...
    assign cache_addr = MemRead_M1 ? alu_out_M1[31:2] : data_addr_aligned;
    ...
    assign rd_in_M1 = (rd_src_M1 == RD_PC4) ? (pc_M1 + 32'h4) : (alu_out_M1);
    ...
    mux #(2, 32) MEM_CACHE_SEL_DATA(.in({cache_read_data_selected_M1, mem_read_data_M1}), .sel(mem_cache_sel_M1), .out(final_data_M1));
    ...
    assign exception_halt   = instr_mem_excpt | data_mem_excpt | illegal_instr_W;
