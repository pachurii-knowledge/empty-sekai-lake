...
(
    ...
    logic                  busy_q;
    ccd_msg_t              msg_q;
    logic [NODE_ID_W-1:0]  dst_q;                       // latched destination (see head_word)
    logic [2:0]            beat_q;                      // 0..4 (0=head, 1..4=body/tail)
    logic [2:0]            ocredit_q [NUM_VC_PHYS];

    logic                  busy_n;
    ccd_msg_t              msg_n;
    logic [NODE_ID_W-1:0]  dst_n;
    logic [2:0]            beat_n;
    ...
    // build the HEAD flit word: { ccd_ctrl, rhdr } zero-extended. Uses the LATCHED dst (dst_q):
    // the producer may change dst_node the cycle after accept (e.g. the directory's per-message
    // out_dst), so the destination must be captured with the message, not read live.
    function automatic logic [CMI_FLIT_W-1:0] head_word();
        ...
        rh.dst=dst_q; rh.src_core=msg_q.src; rh.mclass=mclass;
        d = '0;
        d[CMI_RHDR_W-1:0]              = rh;
    ...
    always_comb begin
        busy_n=busy_q; msg_n=msg_q; dst_n=dst_q; beat_n=beat_q;
        for (int v=0;v<NUM_VC_PHYS;v++) begin
            ocredit_n[v]=ocredit_q[v];
            if (credit_in[v] && ocredit_n[v] < OC_MAX) ocredit_n[v]=ocredit_n[v]+3'd1;
        end
        if (!busy_q) begin
            if (msg_in.valid) begin busy_n=1'b1; msg_n=msg_in.msg; dst_n=dst_node; beat_n=3'd0; end
    ...
    always_ff @(posedge clk or negedge rst_l) begin
        if (!rst_l) begin
            ...
            busy_q<=busy_n; msg_q<=msg_n; dst_q<=dst_n; beat_q<=beat_n;
