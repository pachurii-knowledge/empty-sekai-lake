...
;
    ...
    // ---- shared store/load snoop bus (cores drive these; here the tb does) ----
    logic                          st_en;
    logic [MEMORY_ADDR_WIDTH-1:0]  st_wa;
    logic [XLEN-1:0]               st_wd;
    logic [XLEN_BYTES-1:0]         st_wm;
    logic [MEMORY_ADDR_WIDTH-1:0]  ld_a;
    logic                          ld_en;
    logic [$clog2(XLEN_BYTES)-1:0] ld_off;

    logic                cl_hit;  logic [XLEN-1:0] cl_data;
    logic [3:0]          cl_mtip, cl_msip;   // NUM_HARTS=4
    logic [63:0]         cl_mtime;
    logic                pl_hit;  logic [XLEN-1:0] pl_data;
    logic [3:0]          pl_mext, pl_sext;   // NCTX/2 = 4 harts

    clint #(.NUM_HARTS(4)) CL (
        .clk, .rst_l,
    ...
    // 32-bit load query at a BYTE address (no side effect; settle combinationally).
    task automatic rd32(input logic [31:0] baddr, input bit is_plic,
            output logic [31:0] val);
        ld_a   = baddr[MEMORY_ADDR_WIDTH-1+ADDR_SHIFT:ADDR_SHIFT];
        ld_off = baddr[ADDR_SHIFT-1:0];
        #1;
        val = is_plic ? pl_data[31:0] : cl_data[31:0];
    endtask

    // A PLIC claim load (asserts load_en for one cycle so the gateway closes).
