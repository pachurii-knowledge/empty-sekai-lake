// tb_niigo_ccd_m1.sv -- M1 two-core MOESI mini-system: 2 real L1D agents + the directory +
// a behavioural NMI memory, running an actual cross-core coherence program. Checks cross-core
// visibility, AMO/SC atomicity, false-sharing line integrity, and eviction/refill.
// Build: verilator --binary -DLAB_18447='"4b"' -Isrc -Isrc/mem tests/tb_niigo_ccd_m1.sv --top tb_niigo_ccd_m1
`include "niigo_mem.vh"
`include "niigo_cmi.vh"
`include "niigo_ccd_m1.vh"
`default_nettype none
module tb_niigo_ccd_m1
    ...
    import NIIGO_CCD_M1::*;
;
    localparam int CORES = NIIGO_CMI::NUM_CORES;   // 4 (drive 0,1)
    logic clk=0, rst_l=0;
    always #5 clk=~clk;

    nmi_req_t mreq; logic mreq_ready; nmi_resp_t mresp;

    // ---- two-core CCD subsystem (the M1 wrapper under test) ----
    ...
    niigo_ccd_top #(.NACTIVE(2), .DIR_SETS(8), .L1_SETS(8)) dut (
        .clk, .rst_l,
    ...
    // ---- core LSQ driver tasks (one core at a time; the other stays idle to service snoops) ----
    ...
    // word addresses: V & W share a line (false sharing); Z & Z2 collide on a set (eviction)
    ...
    initial begin
        ...
        $display("== S1: core0 store V=100, core1 load V (cross-core coherence) ==");
        cstore(0, WV, 100);
        ...
        cstore(0, WV, 1);
        cstore(1, WW, 2);                       // core1 takes the line, writes the OTHER word
        ...
        clr  (0, WV, r);                        // core0 reserves V
        cstore(1, WV, 99);                       // core1's write kills core0's reservation
        csc  (0, WV, 88, ok); chk(ok==1'b0, "S5: contended SC FAILS");
        cload(0, WV, r);      chkv(r, 99, "S5: V holds core1's 99, not 88");
        clr  (0, WV, r);                        // uncontended LR/SC
        ...
        cstore(0, WZ, 11);
        cstore(0, WZ2, 22);                      // evicts the Z line (writeback)
        cload (0, WZ, r);  chkv(r, 11, "S6: core0 re-fetches Z after eviction");
        cload (1, WZ2, r); chkv(r, 22, "S6: core1 reads Z2 from core0");

        $display("");
        if (errors==0) $display("==== tb_niigo_ccd_m1: ALL CHECKS PASSED ====");
        else           $display("==== tb_niigo_ccd_m1: %0d CHECK(S) FAILED ====", errors);
        $finish;
    end

    initial begin repeat(6000) @(posedge clk); $display("WATCHDOG TIMEOUT"); $finish; end
endmodule
