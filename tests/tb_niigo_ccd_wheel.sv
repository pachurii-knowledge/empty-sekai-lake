// tb_niigo_ccd_wheel.sv -- M3b: the SAME two-core MOESI coherence program as tb_niigo_ccd_m1,
...
;
    ...
    niigo_ccd_wheel #(.NACTIVE(2), .DIR_SETS(8), .L1_SETS(8)) dut (
        .clk, .rst_l,
    ...
    initial begin
        ...
        $display("== S1: core0 store V=100, core1 load V (cross-core coherence over the wheel) ==");
        cstore(0, WV, 100);
        ...
        $display("");
        if (errors==0) $display("==== tb_niigo_ccd_wheel: ALL CHECKS PASSED ====");
        else           $display("==== tb_niigo_ccd_wheel: %0d CHECK(S) FAILED ====", errors);
        $finish;
    end
