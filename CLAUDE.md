...
- **Memory boundary:** the OoO core speaks three handshaked, latency-agnostic ports (ifetch per 16-byte block, word-granular dmem load/store, PTW req/ack) into `src/mem/niigo_memsys.sv`, currently a passthrough onto `main_memory` reproducing the legacy 2/8-cycle response timing — the seam where the planned L1I/L1D + AXI4 land (`plans/fpga-memsys.md`). The LSQ allows one outstanding load and tracks it (`mem_inflight`/`mem_inflight_kill`), so responses need no address matching at any latency; the in-order cores keep the legacy fixed-latency Lab-4 interface (`ifdef OOO_4WIDE` forks in `riscv_core.sv`/`testbench.sv`).
